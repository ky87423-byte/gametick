...
### 🔴 0. 최우선 — 크롤 회복 확인 (2026-08-07~14 사이, 7/31 조치 후 1~2주)
...
- **판정 분기**:
  ...
  - 크롤 **회복 X** → 응답속도에 남은 원인이 있다. 다음 후보 순서: ①CF 엣지 적중률 실측(cf-cache-status 샘플링) ②오리진 렌더 자체(서버상세 3.5초·HTML 86KB) ③오리진이 말레이시아인 물리적 지연([[cloudflare-cdn]]).
...
- **오리진 MISS 실측(7/31)**: 홈 1.36 / 랭킹 1.24 / 게임 2.85 / 서버상세 **3.52** / 리포트 3.39초, 콜드 최악 **8.8초**. 엣지 HIT는 0.45초라 캐시 적중률로 덮고 있을 뿐 **오리진 자체는 여전히 느리다**.
- 서버상세 HTML **86KB**(홈의 10배) — 줄일 여지 있음.
...
### ⚠️ 다음 세션 진입 전 확인
- 푸시·배포 완료(gametick `d26a64c`·lc_vn `2996eba`). 서버 라이브 정상. (2026-07-31 세션 끝)
...
- **사이트는 Cloudflare 뒤에 있음** — DNS·캐시는 CF 대시보드(계정 ky87423). **HTML 엣지캐싱 30분**(2026-07-31에 5분→30분 상향, `Age 414초 HIT` 검증완료), `/api/*` 제외. ⚠️ **배포 후 변경이 최대 30분 뒤 반영** — 확인이 필요한 배포면 CF Caching→Configuration→**Purge Everything** 하거나 `?v=` 쿼리로 우회 검증. **"Ignore cache-control header" 옵션은 절대 끄지 말 것**(force-dynamic이 no-cache를 보내 캐시가 통째로 안 먹음). [[cloudflare-cdn]]
