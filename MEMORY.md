...
> 마지막 갱신: 2026-07-31 / 작업 일지: `docs/worklog.md` / 계획서: `PLAN.md`
> **세션 시작 시 이 파일부터 읽으세요.** (다음 할 일은 worklog 맨 끝 "다음 세션 할 일")
>
> ## 🔴 2026-07-31 요약 — 다음 세션은 여기부터
>
> **한 줄**: 7월 노출 붕괴(-96.9%)의 원인이 **서버 응답속도**로 밝혀졌다. `/api/*` 크롤 차단 + CF 엣지 TTL 30분 상향으로 조치했고, **1~2주 뒤 크롤링 통계 회복 여부가 다음 세션의 첫 확인 사항**이다.
>
> - 🔑 **원인 규명**: 크롤 요청 7/1 **7,004회 → 7/5 17회**(-99.8%), 평균 응답시간 241ms → 1,414ms(7/13엔 **14,463ms**), GSC 호스트 상태 "이전에 문제 있었음". **크롤 붕괴(7/3~5)가 노출 붕괴(7/13)보다 10일 앞선다** → 인과는 `느린 응답 → 구글이 크롤속도 자동 인하 → 신선도 상실 → 노출 붕괴`. **CTR 저하는 원인이 아니라 결과**였다(세션 중반의 "핵심=CTR" 판단을 크롤통계가 뒤집음).
> - 🔑 **범인 = `/api/*`**: 크롤 요청의 **85.29%가 HTML 아닌 형식**(HTML 10.8%). `/api/*`는 실시간 폴링 보호로 CF 캐시 제외(`DYNAMIC`)라 매 요청 오리진 직행 **1.9~5.2초**인데 robots.txt가 허용 중이었다. 색인 가치는 0. → `robots.ts` **`Disallow: /api/`**(`ff0eebd`).
> - **CF 엣지 TTL 300초 → 1800초(30분)** 상향 완료·검증(`Age 414초 HIT`). ⚠️ **배포 반영이 최대 30분 뒤** — 급하면 CF Purge Everything.
> - **사이트맵**: 3,178 URL·2.87MB·**6.2초** → 832 URL·750KB·**0.8초**(ko 전용화 `addKo` + `makeTtlCache` 30분 SWR, `3169ff7`). 구글봇이 매번 밟던 느린 URL이었다.
> - **제목·스니펫**(이전 세션이 커밋조차 안 하고 방치한 것을 발견해 배포): 서버 제목 `데포로쥬 아데나 시세 | 리니지 클래식 아데나 시세 - 게임시세`(gamebit 패턴), meta description 앞머리에 **실가격+서버수**.
> - **게임 페이지 보강**(`c67ea63`): 실데이터 **시세 요약문**(`gameSummaryText`, 7개 언어) + **가격 FAQ**(`priceFaqItems`) + **Product/AggregateOffer** JSON-LD(순위 상위 adena.kr도 같은 스키마, gamebit은 WebSite만) + h2 키워드 헤딩. 부수 수정: `MarketSummary.nameEn`(외국어판 한글 서버명), 한국어 **조사 `josa()`**("아데나이"), **"기타"=바로템 미분류 버킷을 서버로 단정 금지**.
> - **게임별 실순위**(GSC): 리니지2M 6.8위(1페이지) / 솔인챈트 23.9위 / 아이온2 26.0위 / 리니지클래식 **45.6위**(5페이지) / 아스달연대기 데이터 0(7/27 추가). 클릭이 나오는 곳은 **신작·틈새**(솔인챈트 6·아이온2 5클릭), 리니지클래식은 격전지.
> - **서버 URL 슬러그화는 의도적 보류** — 대상 롱테일이 5페이지라 회수가 느린데 색인된 399페이지 URL 변경 리스크가 크다. 크롤 회복 후 재검토.
> - ⚠️ **진단 순서 교훈**: 실적(Performance)만 보면 원인을 잘못 짚는다. **크롤통계 → 색인 → 실적** 순으로 볼 것. memory `gsc-index-status`·`cloudflare-cdn` 갱신됨.
>
> **2026-07-27 요약**(worklog "2026-07-27"):
