...
- **네임드 순위는 여전히 수동 큐레이션**(`data/rankings.ts`의 RANKINGS, 비면 "준비 중").
  깔끔한 자동 소스 없음 → 채우려면 게임 랭킹사이트 스크래핑 또는 수동 입력.
...
**실동작**: 시세표(검색·정렬·즐겨찾기) · 자동갱신 폴링(`/api/prices`, 60s, 탭 숨김 시 중단) · 서버상세 캔들차트(3분/1시간/일봉 + 이동평균선) · 24h 등락 · 매물수 표시 · 브라우저 가격알림(localStorage) · 즐겨찾기 대시보드 · SEO(페이지별 메타+sitemap+robots) · 임베드 위젯 · VND 환산(vi) · PWA manifest · umami(env로 켬) · 다크테마/한국색상 · **BJ 순위(치지직 라이브 실시간)**.

**스텁/대기**: 멀티거래소(추상화만, barotem 1곳) · 커뮤니티 위젯(빈 상태) · 텔레그램/디스코드 알림(워커 필요) · **네임드 순위(수동 큐레이션, 현재 빈 상태)**.
